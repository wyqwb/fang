...
<div class="ADD_coninp">
    <form action="./admin/product/update_income" method="post">
        <table class='lighttable' width="100%" cellpadding="0" cellspacing="0">
            <tbody>
            <tr><input type='hidden' name='id' value="<?php if(isset($result['id'])){echo $result['id'];}?>" /><th width="25%">产品编码：</th><td width="75%"><input name='number' value='<?php if(isset($number)){echo $number;}?>' readonly="readonly" type="text" ></td></tr>
            <tr><th>收益率：</th><td><input name='income' value='<?php if(isset($result['income'])){echo $result['income'];}?>' type="text"></td></tr>
            <tr><th>创建日期：</th><td><input class='datePicker' name='ctime' value='<?php if(isset($result['ctime'])){echo $result['ctime'];}?>' type="text"></td></tr>
