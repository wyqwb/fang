...
class CI_Table {
	...
	var $rows				= array();
	var $heading			= array();
	var $footing_arr        = array();
	var $footing_str        = "";
	var $foot_colspan       = 30;
	var $auto_heading		= TRUE;
	var $auto_footing       = FALSE;
	var $caption			= NULL;
	var $template			= NULL;
	var $newline			= "\n";
	var $empty_cells		= "";
	...
	{
		$args = func_get_args();
		$this->heading = $this->_head_prep_args($args);
	}
	
	function set_footing()
	{
		$args = func_get_args();
		$this->footing_arr = $this->_foot_prep_args($args);
	}
	// --------------------------------------------------------------------
	 ...
	 * @return	type
	 */
	private function _head_prep_args($args)
	{
	...
	private function _foot_prep_args($args)
	{
		if (isset($args[0]) AND (count($args) == 1 && is_array($args[0])))
		{
			// args sent as indexed array
		...
		else
		{
			...
			{
				$this->auto_footing = TRUE;
				$this->foot_colspan = $args[1];
				$this->footing_str = $args[0];
			}
			else
			{
				foreach ($args as $key => $val)
	...
	function generate($table_data = NULL)
	{
			...
			foreach ($this->heading as $heading)
			{
				...
				foreach ($heading as $key => $val)
				{
					if(is_array($val))
					{
						...
						{
							if($key2 != 'data' && $key2 != 'value')
							{
								$temp = str_replace('<th',"<th $key2='$val2'",$temp);
							}
						}
					}
					else
		...
		if($this->auto_footing == TRUE)
		{
			$out .= $this->template['tfoot_open'];
			$out .= $this->newline;
			$out .= $this->template['footing_row_start'];
			$out .= $this->newline;
			$out .= "<th colspan='".$this->foot_colspan."'>" ; 
			$out .= $this->footing_str;
